...
for network_os in list(compliance.parser_map.keys()):
    section_config_parameters.extend(
        [_file, network_os]
        for _file in glob.glob(f"{MOCK_DIR}/section/{network_os}/*{TXT_FILE}")
    )

    compliance_parameters.extend(
        [_file, network_os]
        for _file in glob.glob(
            f"{MOCK_DIR}/compliance/{network_os}/*{INTEND_FILE}"
        )
    )

    diff_network_config_parameters.extend(
        [_file, network_os]
        for _file in glob.glob(
            f"{MOCK_DIR}/diff_network_config/{network_os}/*{INTEND_FILE}"
        )
    )

    config_section_not_parsed_parameters.extend(
        [_file, network_os]
        for _file in glob.glob(
            f"{MOCK_DIR}/config_section_not_parsed/{network_os}/*{TXT_FILE}"
        )
    )
...
def test_section_config(_file, network_os, get_text_data, get_python_data):  # pylint: disable=redefined-outer-name
    ...
    received_data = get_text_data(f"{truncate_file}_received.txt")
    feature = get_python_data(f"{truncate_file}_feature.py", "feature")
...
def test_compliance(
    ...
    backup_cfg = get_text_data(f"{truncate_file}_backup.txt")
    received_data = get_json_data(f"{truncate_file}_received.json")
    features = get_python_data(f"{truncate_file}_feature.py", "features")
...
def test_diff_network_config(_file, network_os, get_text_data):  # pylint: disable=redefined-outer-name
    ...
    base_config = get_text_data(f"{truncate_file}_base.txt")
    received_data = get_text_data(f"{truncate_file}_received.txt")
...
def test_find_unordered_cfg_lines(_file, get_text_data, get_python_data):
    ...
    actual_config = get_text_data(f"{truncate_file}_actual.txt")
